...
export function OrderStatusTimeline({ order }: { order: Order }) {
  if (order.status === "cancelled") {
    return (
      <div className="border border-red-500/30 bg-red-500/5 p-8 text-center">
        ...
        <h2 className="mt-3 font-serif text-2xl text-navy">Order {order.reference}</h2>
  ...
  return (
    <div className="border border-hairline p-6 sm:p-8">
      <p className="text-xs uppercase tracking-[0.25em] text-gold-ink">Order Status</p>
      <h2 className="mt-2 font-serif text-2xl text-navy">Order {order.reference}</h2>
            ...
            <li key={step.status} className="flex items-start gap-4">
              <span
                className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border text-[11px] ${
                  done ? "border-gold bg-gold text-black" : "border-hairline text-muted/70"
                }`}
              >
                {index + 1}
              </span>
              <div>
                <p className={done ? "text-navy" : "text-muted"}>{step.label}</p>
